(function() {
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function(cur, item) {
      if(!iterator(item)) {
        cur = false;
      }
      return cur;
 ...
 _.memoize = function(func) {
  ...
  _.shuffle = function(array) {
    ...
    for(var i = 0; i < arr.length; i++) {
      var newLoc = Math.round(Math.random() * (arr.length - 1));
      var arr1 = arr[newLoc];
      arr[newLoc] = arr[i];
      arr[i] = arr1;
    }
  ...
  _.zip = function() {
    ...
    for(var i = 0; i < longestArray; i++) {
      for(var j = 0; j < arguments.length; j++) {
        zippedPrevious.push(arguments[j][i])
      }
      zipped.push(zippedPrevious);
      zippedPrevious = [];
    }
    return zipped;
  };
  ...
  _.flatten = function(nestedArray, result) {
    return _.reduce(nestedArray, function (flat, toFlatten) {
      if(Array.isArray(toFlatten)) {
        return flat.concat(_.flatten(toFlatten))
      } else {
